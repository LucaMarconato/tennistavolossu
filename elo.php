...
$res = $database->cmd_my_sql("SELECT giocatore1 FROM partite WHERE matchCode = '".addslashes($matchCode)."'",__FILE__,__LINE__);
$fetched = mysql_fetch_array($res);
$idGiocatore1 = $fetched['giocatore1'];

$res = $database->cmd_my_sql("SELECT giocatore2 FROM partite WHERE matchCode = '".addslashes($matchCode)."'",__FILE__,__LINE__);
$fetched = mysql_fetch_array($res);
$idGiocatore2 = $fetched['giocatore2'];

$res = $database->cmd_my_sql("SELECT punteggio1 FROM partite WHERE matchCode = '".addslashes($matchCode)."'",__FILE__,__LINE__);
$fetched = mysql_fetch_array($res);
$punteggio1 = $fetched['punteggio1'];

$res =$database->cmd_my_sql("SELECT punteggio2 FROM partite WHERE matchCode = '".addslashes($matchCode)."'",__FILE__,__LINE__);
$fetched = mysql_fetch_array($res);
...
$res = $database->cmd_my_sql("SELECT ranking FROM utenti WHERE id = '".addslashes($idGiocatore2)."'",__FILE__,__LINE__);
$fetched = mysql_fetch_array($res);
